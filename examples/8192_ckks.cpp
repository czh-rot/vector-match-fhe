/******************************************************************************
 *  CKKS 向量模糊匹配（Cosine，L2 归一化版本，Diagonal + BSGS）
 *
 *  通用设置：
 ...
 *    - OpenMP 并行
 *    - 只支持 Cosine，相似度使用 L2 归一化 embedding（Test 模式不用归一化）
 *
 *  CKKS 参数（与当前可用版本保持一致）：
 *    - CKKS_SCALE             = 2^40
 *    - coeff_modulus          = { 60, 40, 40, 60 }
 *    - 每次 multiply_plain 后：rescale_to_next + 手动把 scale 设回 2^40
 *    - add 前通过 mod_switch_to + 对齐 scale（add_aligned_inplace）
 *
 *  本文件提供：
 *    1) vector_match_ckks_naive_diag  —— 朴素对角线实现（DIM 次旋转）
 *    2) vector_match_ckks_bsgs        —— BSGS 实现（≈ 2*sqrt(DIM) 级旋转）
 *    3) main 里分别调用两者做对比
 ******************************************************************************/
...
/* 1D 槽向量右移（BSGS 预补偿用） */
static inline void shift_slots_cyclic(std::vector<double> &v, size_t sft) {
    size_t n = v.size();
    if (!n) return;
    sft %= n;
    if (!sft) return;
...
/*
 * BSGS 编码：在 naive 对角线基础上做「巨步预补偿」。
 *
 * naive:   P_{s,d}[i] = E[gid][(i + d) % DIM]
 * BSGS:    d = j*baby + k，预编码 DB_bsgs_{s,d} = RotRight(P_{s,d}, r)
 *          r = (j * baby) % slot_count
 *
 * 运行时：
 *   - baby-step: Q_k = Rot(Q, k)
 *   - inner(j):  acc_j = Σ_k rescale(Q_k * DB_bsgs_{s, j*baby+k})
 *   -          row_acc += Rot(acc_j, j*baby)
 *
 * 利用性质 Rot(A, r)⊙Rot(B, r) = Rot(A⊙B, r)，可以证明最终和 naive 完全等价。
 */
std::vector<std::vector<Plaintext>> encode_database_diagonal_ckks_bsgs(
    const SEALContext &context,
    const std::vector<std::vector<double>> &embeddings,
    size_t slot_count,
    size_t dim,
    ...
    size_t TOP_K)
{
    ...
    SEALContext context(parms);
    if (!context.parameters_set()) {
        throw std::runtime_error("[CKKS-BSGS] encryption parameters are not valid.");
    }
    ...
    if (DIM > slot_count || (slot_count % DIM) != 0) {
        throw std::invalid_argument(
            "[CKKS-BSGS] require DIM <= slot_count and slot_count % DIM == 0.");
    }
    const size_t fd = (NUM_EMBEDDINGS + slot_count - 1) / slot_count;
    const auto [baby, giant] = plan_bsgs(DIM);

    std::cerr << "[CKKS-BSGS] poly_degree=" << POLY_DEGREE
              << ", slot_count=" << slot_count
              << ", NUM_EMBEDDINGS=" << NUM_EMBEDDINGS
              << ", DIM=" << DIM
    ...
    auto db = encode_database_diagonal_ckks_bsgs(
    ...
    auto t1 = std::chrono::high_resolution_clock::now();
    auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
    std::cout << "[CKKS-BSGS] Compute time: " << ms << " ms\n";

    if (data_mode == EmbMode::Test) {
        if (result.empty()) {
            std::cout << "[CKKS-BSGS TEST] result empty.\n";
            return;
        }
        ...
        std::cout << "[CKKS-BSGS TEST] first 16 slots (gid, expected, decrypted):\n";
        size_t print_cnt = std::min<size_t>(16, slots.size());
        for (size_t i = 0; i < print_cnt; ++i) {
            size_t gid      = i;
            double expected = static_cast<double>(DIM); // Test: 1·1=1, sum DIM 次
            double dec_val  = slots[i];
    ...
    std::cout << "[CKKS-BSGS] Top-" << TOP_K
              << " overlap (HE vs Plain): "
              << intersect_cnt << " / " << TOP_K << "\n";
}

/* ------------------- 示例 main：依次跑 naive & BSGS ------------------- */
...
int vector_match() {
    ...
    // 1) 先跑 BSGS Test，确认 correctness（可选）
    /*
    vector_match_ckks_bsgs(
