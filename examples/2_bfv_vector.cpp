...
/* ------------------- 示例 demo（BGV） ------------------- */
/* ------------------- Paper-style benchmark driver: BGV ------------------- */

static const char* bgv_mode_name(EmbMode m) {
    switch (m) {
...
static const char* bgv_metric_name(Metric m) {
    switch (m) {
        case Metric::Cosine: return "Cosine";
        case Metric::Euclid: return "Euclid";
        default:             return "Unknown";
    }
}

static bool run_bgv_case(
    ...
    Metric metric)
{
    std::cerr << "\nBENCH_BEGIN"
              ...
              << ",D=" << dim
              << ",threads=" << threads
              << ",plain_bits_hint=" << plain_modulus_bits
              << ",mode=" << bgv_mode_name(data_mode)
              << ",qmode=" << bgv_mode_name(query_mode)
              << ",metric=" << bgv_metric_name(metric)
              << ",scale=" << scale_int
              << ",topk=" << top_k
              << ",l2=" << (l2_normalize ? 1 : 0)
              << std::endl;
    ...
    try {
        vector_match_bgv(
            poly_degree, num_embeddings, dim, threads, plain_modulus_bits,
            data_mode, query_mode,
            data_seed, query_seed,
            scale_int, top_k,
            l2_normalize, metric);
...
int main2_bgv() {
    ...
    const size_t PLAIN_BITS_HINT = 20;
    ...
    // 0. Sanity check
    failed += !run_bgv_case(
        "sanity", "test_N1024_D128",
        POLY, 1024, 128, 64, PLAIN_BITS_HINT,
        EmbMode::Test, EmbMode::Test,
        DATA_SEED, QUERY_SEED,
        64, 16, true, Metric::Cosine);

    // 1. Default baseline
    failed += !run_bgv_case(
        "baseline", "N4096_D1024_scale512",
        POLY, 4096, 1024, 64, PLAIN_BITS_HINT,
        EmbMode::Uniform, EmbMode::Uniform,
        DATA_SEED, QUERY_SEED,
        512, TOP_K, true, Metric::Cosine);

    // 2. Quantization sensitivity
    for (long long S : {64LL, 128LL, 256LL, 512LL, 1024LL}) {
        failed += !run_bgv_case(
            "scale_sensitivity", "scale" + std::to_string(S),
            POLY, 4096, 1024, 64, PLAIN_BITS_HINT,
            EmbMode::Uniform, EmbMode::Uniform,
            DATA_SEED, QUERY_SEED,
            S, TOP_K, true, Metric::Cosine);
    }
    ...
    for (size_t N : {2048ULL, 4096ULL, 8192ULL, 16384ULL}) {
        failed += !run_bgv_case(
            ...
            POLY, N, 1024, 64, PLAIN_BITS_HINT,
            EmbMode::Uniform, EmbMode::Uniform,
            DATA_SEED, QUERY_SEED,
            512, TOP_K, true, Metric::Cosine);
    }

    // 4. Dimension scalability
    for (size_t D : {128ULL, 256ULL, 512ULL, 1024ULL}) {
        failed += !run_bgv_case(
            "dim_scaling", "N4096_D" + std::to_string(D),
            POLY, 4096, D, 64, PLAIN_BITS_HINT,
            EmbMode::Uniform, EmbMode::Uniform,
            DATA_SEED, QUERY_SEED,
            512, TOP_K, true, Metric::Cosine);
    }
    ...
    for (size_t T : {1ULL, 2ULL, 4ULL, 8ULL, 16ULL, 32ULL, 64ULL}) {
        failed += !run_bgv_case(
            "thread_scaling", "T" + std::to_string(T),
            POLY, 4096, 1024, T, PLAIN_BITS_HINT,
            EmbMode::Uniform, EmbMode::Uniform,
            DATA_SEED, QUERY_SEED,
            512, TOP_K, true, Metric::Cosine);
    }
    ...
    failed += !run_bgv_case(
        ...
        POLY, 4096, 1024, 64, PLAIN_BITS_HINT,
        EmbMode::Gaussian, EmbMode::Gaussian,
        DATA_SEED, QUERY_SEED,
        512, TOP_K, true, Metric::Cosine);
