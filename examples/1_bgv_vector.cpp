...
/* ------------------- Paper-style benchmark driver: BFV ------------------- */

static const char* bfv_mode_name(EmbMode m) {
    switch (m) {
...
static const char* bfv_metric_name(Metric m) {
    switch (m) {
    ...
    Metric metric)
{
    std::cerr << "\nBENCH_BEGIN"
              ...
              << ",mode=" << bfv_mode_name(data_mode)
              << ",qmode=" << bfv_mode_name(query_mode)
              << ",metric=" << bfv_metric_name(metric)
              << ",scale=" << scale_int
              << ",topk=" << top_k
              << ",l2=" << (l2_normalize ? 1 : 0)
              << std::endl;
